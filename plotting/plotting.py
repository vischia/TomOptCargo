...
def get_passives_xy_min_max(volume: Volume) -> np.ndarray:
    ...
    return np.array([0.0, xmax, 0.0, ymax])
...
def plot_hits(
    ...
    ax.set(
        xlim=[xy_min_max[dim] - gap, xy_min_max[dim + 1] + gap],
        ylim=[zmin - gap, zmax + gap],
    )
        ...
        if isinstance(l, HodoscopeDetectorLayer):
            for i, p in l.yield_zordered_panels():
                ...
                ax.axhline(
                    ...
                    alpha=0.6,
                    xmin=relative_pos(
                        ax, (p.xy[dim] - p.xy_span[0] / 2).detach().item()
                    ),
                    xmax=relative_pos(
                        ax, (p.xy[dim] + p.xy_span[0] / 2).detach().item()
                    ),
...
def draw_volume_2D(
    ...
    xy_min = np.where(
        xy_min_max_panels < xy_min_max_passives, xy_min_max_panels, xy_min_max_passives
    )
    xy_max = np.where(
        xy_min_max_panels > xy_min_max_passives, xy_min_max_panels, xy_min_max_passives
    )
    ...
    for l in volume.layers:
        if isinstance(l, HodoscopeDetectorLayer):
            for h in l.hodoscopes:
                hods.append(
                    {
                        "x": h.xy[0].detach().item()
                        - h.xyz_span[0].detach().item() / 2,
                        "y": h.xy[1].detach().item()
                        - h.xyz_span[1].detach().item() / 2,
    ...
    # XZ view
    XZ_hods = [Rectangle((hod["x"], hod["z"]), hod["dx"], hod["dz"]) for hod in hods]
    # plot hodoscope
    axs[0].add_collection(
        PatchCollection(XZ_hods, facecolor="green", alpha=0.2, edgecolor="green")
    )
    # plot passive
    axs[0].add_collection(
        PatchCollection(
    ...
    axs[1].add_collection(
        PatchCollection(YZ_hods, facecolor="green", alpha=0.2, edgecolor="green")
    )
    # plot passives
    axs[1].add_collection(
        PatchCollection(
    ...
    # XY view
    # plot hodoscopes
    XY_hods = [Rectangle((hod["x"], hod["y"]), hod["dx"], hod["dy"]) for hod in hods]

    axs[2].add_collection(
        PatchCollection(XY_hods, facecolor="green", alpha=0.2, edgecolor="green")
    )

    # plot panels
    XY_planes = [
        Rectangle((p["x"] - p["dx"] / 2, p["y"] - p["dy"] / 2), p["dx"], p["dy"])
        for p in panels
    ]

    axs[2].add_collection(
        PatchCollection(XY_planes, facecolor="red", alpha=0.2, edgecolor="red")
    )
    ...
    if hits is not None:
        axs[0].scatter(
            hits[event, :, 0].detach().numpy(),
            hits[event, :, 2].detach().numpy(),
            marker="+",
        )
        axs[1].scatter(
            hits[event, :, 1].detach().numpy(),
            hits[event, :, 2].detach().numpy(),
            marker="+",
        )
        axs[2].scatter(
    ...
    if pocas is not None:
        axs[0].scatter(
            pocas[event, 0].detach().numpy(),
            pocas[event, 2].detach().numpy(),
            marker="+",
        )
        axs[1].scatter(
            pocas[event, 1].detach().numpy(),
            pocas[event, 2].detach().numpy(),
            marker="+",
        )
        axs[2].scatter(
