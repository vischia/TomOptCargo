            ...
            for axes, det in zip([self.above_det, self.below_det], self.wrapper.get_detectors()):
                ...
                for h in range(len(loc)):
                    ...
                    for p in range(len(p_loc)):
                        axes[0].add_line(
                            mlines.Line2D((p_loc[p, 0] - (p_span[p, 0] / 2), p_loc[p, 0] + (p_span[p, 0] / 2)), (p_loc[p, 2], p_loc[p, 2]), linewidth=2, color=palette[p])
                        ) # xz
                        axes[1].add_line(
                            mlines.Line2D((p_loc[p, 1] - (p_span[p, 1] / 2), p_loc[p, 1] + (p_span[p, 1] / 2)), (p_loc[p, 2], p_loc[p, 2]), linewidth=2, color=palette[p])
                        )# yz
