...
uranium = X0['uranium']
lead = X0['lead']
del X0['uranium'] ; del X0['lead'] ; del X0['water'] ; del X0['air'] ; del X0['beryllium']
...
class RandomPassive:
    ...
    There are three preconfigured passive options:
        - simple: <1 m^3 volume of a random material with a block (not more) of another random material.
        - medium: cube of 1-2 m sides with 3 (customisable) random material blocks inside.
        - complex: truck-like passive with steel frame and dimensions 6 x 2.5 x 2.5 m^3 and 15 random material blocks inside.
    ...
    def __init__(self,
                 lwh: Tensor = None,
                 n_blocks: int = None,              # only for medium/complex/custom
                 heavy_material_chance: float = .5, # lead/uranium
                 truck_frame: bool = True,          # only for complex/custom
    ...
    def simple(self) -> Union[Tuple[Tensor, str], Tuple[List[PassiveLayer], Tensor, str]]:
        ...
        if self.heavy_material_chance != False:
            if random.choice(np.arange(0, 1, self.heavy_material_chance)) == False:
                self.rm[0] = random.choice([uranium, lead])
                self.should_open = 1.
        ...
        def generator(*, 
            ...
            rad_length = torch.ones(list((lw / size).long()))*rm[-1]
            ...
            if z >= block_low[2] and z <= block_high[2]:
                rad_length[low_xy[0] : high_xy[0], low_xy[1] : high_xy[1]] = rm[0]
    ...
    def medium(self) -> Union[Tuple[Tensor, str], Tuple[List[PassiveLayer], Tensor, str]]:
    ...
    def complex(self) -> Union[Tuple[Tensor, str], Tuple[List[PassiveLayer], Tensor, str]]:
    ...
    def custom(self) -> Union[Tuple[Tensor, str], Tuple[List[PassiveLayer], Tensor, str]]:
        ...
        if self.heavy_material_chance != False:
            if random.choice(np.arange(0, 1, self.heavy_material_chance)) == False:
                self.rmidx[0] = random.choice([uranium, lead])
                self.should_open = 1.
    ...
    def gen_passive_layers(self) -> List[PassiveLayer]:
        ...
        for z in np.arange(self.lwh[-1], 0, -self.voxel_size):
            layers.append(PassiveLayer(lw=self.lwh[:2], 
                                       z=z, 
